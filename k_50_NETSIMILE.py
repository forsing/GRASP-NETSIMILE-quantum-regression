...
# Varijanta: NETSIMILE – potpis čvora (stepen, clustering, statistike suseda), SVD → k dim

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
NETSIMILE: za svaki čvor potpis = (stepen, clustering coeff, prosek stepena suseda,
std stepena suseda, ...); matrica (39×d) → SVD na k dimenzija → [0, π].
"""
...
NETSIMILE_N = 5 # 5 components for NETSIMILE
...
def compute_netsimile_embeddings(M, k=EMBED_DIM):
    G = nx.Graph()
    for i in range(1, 40):
        G.add_node(i)
    for i in range(1, 40):
    ...
    n = 39
    H = np.zeros((n, 5), dtype=float)
    for i in range(1, 40):
        idx = i - 1
        H[idx, 0] = deg.get(i, 0)
        H[idx, 1] = cc.get(i, 0.0)
        neigs = list(G.neighbors(i))
        if neigs:
            neig_degs = [deg.get(j, 0) for j in neigs]
            H[idx, 2] = np.mean(neig_degs)
            H[idx, 3] = np.std(neig_degs) if len(neig_degs) > 1 else 0.0
            H[idx, 4] = np.mean([cc.get(j, 0.0) for j in neigs])
    U, S_svd, _ = np.linalg.svd(H, full_matrices=False)
...
def main():
    ...
    emb = compute_netsimile_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (NETSIMILE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (NETSIMILE + kvantna regresija, deterministički, strukturno):
    (8, 11, 22, 23, 26, 28, 33)

    Score: 0.5172651554853415
    """
...
Graf: 
NetworkX iz ko-okarence, težine na granama (39×39).

Potpis po čvoru (5 komponenti): 
(1) stepen, (2) clustering (weighted), (3) prosek stepena suseda, (4) std stepena suseda, (5) prosek clustering-a suseda (39×5).

SVD: 
H (39×5) → prvih k levih singularnih vektora × √σ; ako k > 5, dopuna poslednjom kolonom (39×k).
